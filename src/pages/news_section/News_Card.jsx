...
const News_Card = () => {
  ...
  const CurrentTab =
    activeTab === "all"
      ? allNewsData
      : allNewsData?.filter((food) => food?.category === activeTab);
        ...
        <section className="container mx-auto   w-full grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 md:gap-4 gap-2 md:px-2 py-4 ">
          {CurrentTab?.map((news) => (
            <div key={news._id} className="">
              <div className="w-full lg:h-[65vh] bg-black border border-gray-500 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <img
                  ...
                  <button className="inline-flex items-center px-3 py-2 text-md font-medium text-center text-[#FF9D00] rounded-lg focus:ring-1 focus:outline-none">
                    ...
                    <svg
                      className={`w-3.5 h-3.5 ms-2 ${
                        expandedNews[news._id] && "rtl:rotate-180"
                      }`}
                      aria-hidden="true"

...
const LikeAndDislike = ({ newsData }) => {
  ...
  const handleLike = async () => {
    if (!isLiked) {
      ...
      try {
        await instance.patch(`/api/v1/news/${newsData._id}`, {
          likes: likes + 1,
        });
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-6 mt-2 ">
        ...
        >
          <GrDislike
            className={`md:w-4 md:h-4 w-3 h-3 text-${likeColor}-700 `}
          ></GrDislike>
        </button>
      </div>
    </div>
